...
export default function CustomersPage() {
  ...
  }, [searchTerm, page, token, user]); // Will refetch on search term or page change
  ...
  const loadStaff = async (search: string, currentPage: number) => {
    ...
    setIsLoading(true);
    try {
      const result = await api.listStaff(token, { role: 'CUSTOMER', search, page: currentPage, limit });
  ...
  if (authLoading) {
    return <div className="p-8 text-gray-500">Loading customers...</div>;
  }
